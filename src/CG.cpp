...
//#endif
  ...
  Vector & Ap = data.Ap;
	...
	MPI_Comm comm = A.comm; // MPI Comm added 	
  ...
  TICK(); ComputeDotProduct(nrow, r, r, normr, t4, A.isDotProductOptimized, comm); TOCK(t1);
  normr = sqrt(normr);
#ifdef HPCG_DEBUG
  ...
  for (int k=1; k<=max_iter && normr/normr0 > tolerance; k++ ) {
    ...
    if (k == 1) {
      ...
      TICK(); ComputeDotProduct (nrow, r, z, rtz, t4, A.isDotProductOptimized, comm); TOCK(t1); // rtz = r'*z
    } else {
      oldrtz = rtz;
      TICK(); ComputeDotProduct (nrow, r, z, rtz, t4, A.isDotProductOptimized, comm); TOCK(t1); // rtz = r'*z
      beta = rtz/oldrtz;
    ...
    TICK(); ComputeSPMV(A, p, Ap); TOCK(t3); // Ap = A*p
    TICK(); ComputeDotProduct(nrow, p, Ap, pAp, t4, A.isDotProductOptimized, comm); TOCK(t1); // alpha = p'*Ap
    alpha = rtz/pAp;
